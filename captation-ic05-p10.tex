...
\institute{camille.maussang@linkfluence.net\\ linkfluence}
\date{IC05 - P10}
...
\frame{
	...
	\uncover<+->{\rtgicolor<.>{n'importe qui, n'importe où (ouvert)}}
...
\frame{
    ...
    \item Web dynamique (syndication : blogs, médias) : du flux RSS à PubSubHubbub
	\item Web applicatif (réseaux sociaux, sites de contenus, micropublications)
    \item Apparition de nouveaux protocoles : XMPP
	\end{itemize}
	\end{block}
}
...
\frame{
    ...
    \item Ne capter que ce qui est nécessaire
    \end{itemize}
	\end{block}
...
\frame{
    ...
    \item \url{http://en.wikipedia.org/wiki/Atom_%28standard%29}
